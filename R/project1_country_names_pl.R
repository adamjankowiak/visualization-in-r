...
# Pipeline generujący dane i artefakty dashboardu Shiny
# Plik jest bezpieczny dla Shiny: po source() tylko definiuje funkcję.
# Aby uruchomić pipeline: run_project1_country_names_pl()
# ============================================================

run_project1_country_names_pl <- function(project_root = NULL) {
  ...
  # 1A. Ścieżki projektu
  # ============================================================
  # Skrypt jest przeznaczony do uruchamiania w strukturze projektu:
  # - dane wejściowe i eksporty CSV: data/
  # - wykresy HTML/PNG używane przez Shiny: www/
  # Dzięki temu pliki wynikowe trafiają od razu do folderów używanych przez app.R.
  
  if (is.null(project_root)) {
    project_root <- {
      wd <- normalizePath(getwd(), winslash = "/", mustWork = TRUE)
      if (dir.exists(file.path(wd, "data")) && dir.exists(file.path(wd, "R"))) {
        wd
      } else if (basename(wd) == "R" && dir.exists(file.path(dirname(wd), "data"))) {
        dirname(wd)
      } else {
        wd
      }
  ...
  www_dir <- file.path(project_root, "www")
  
  dir.create(data_dir, recursive = TRUE, showWarnings = FALSE)
  dir.create(www_dir, recursive = TRUE, showWarnings = FALSE)
  
  input_catalog_path <- file.path(data_dir, "IKEA_product_catalog.csv")
  
  if (!file.exists(input_catalog_path)) {
    stop(
      paste0(
        "Brakuje pliku wejściowego: ", input_catalog_path,
        "\nUmieść plik IKEA_product_catalog.csv w folderze data/ i uruchom skrypt ponownie."
      )
    )
  }
  ...
  IKEA_product_catalog <- read_csv(input_catalog_path)
  ...
  exchange_rates_cache_path <- file.path(
    data_dir,
    paste0("exchange_rates_eur_", exchange_date, ".csv")
  )
  
  perform_get_json <- function(url, max_tries = 3, pause_sec = 2) {
    last_error <- NULL
    ...
    for (try_no in seq_len(max_tries)) {
      result <- tryCatch(
        {
          ...
          list(ok = TRUE, response = response)
        },
        error = function(e) {
      ...
      if (isTRUE(result$ok)) {
        return(resp_body_json(result$response))
      }
  
      last_error <- result$error
  
      if (try_no < max_tries) {
        Sys.sleep(pause_sec * try_no)
      }
    }
  ...
  read_rates_cache <- function(path) {
    read_csv(path, show_col_types = FALSE) |>
      mutate(
        currency = str_to_upper(str_squish(currency)),
        rate_per_eur = as.numeric(rate_per_eur)
      ) |>
      filter(
        !is.na(currency),
        !is.na(rate_per_eur),
  ...
  fetch_exchange_rates <- function(url, cache_path) {
    fetched <- tryCatch(
      {
        rates_json <- perform_get_json(url)
        ...
        tibble(
          ...
          mutate(currency = str_to_upper(str_squish(currency))) |>
          distinct(currency, .keep_all = TRUE)
      },
      error = function(e) {
        if (file.exists(cache_path)) {
          warning(
            paste0(
              "Nie udało się pobrać kursów walut z API Frankfurter. ",
              "Używam lokalnego cache: ", cache_path,
              "\nSzczegóły błędu: ", conditionMessage(e)
            ),
            call. = FALSE
          )
          return(read_rates_cache(cache_path))
        }
  
        stop(
          paste0(
            "Nie udało się pobrać kursów walut z API Frankfurter i nie znaleziono cache: ",
            cache_path,
            "\nUruchom skrypt ponownie później albo dodaj plik cache z kolumnami: ",
    ...
    write_csv(fetched, cache_path)
    fetched
  }
  
  api_rates_tbl <- fetch_exchange_rates(url, exchange_rates_cache_path)
  ...
  ppp_cache_path <- file.path(data_dir, "ppp_conversion_factors_private_consumption.csv")
  
  read_ppp_cache <- function(path) {
    read_csv(path, show_col_types = FALSE) |>
      mutate(
        iso3 = str_to_upper(str_squish(iso3)),
        ppp_year = as.integer(ppp_year),
        ppp_lcu_per_int_dollar = as.numeric(ppp_lcu_per_int_dollar)
      ) |>
      filter(
        !is.na(iso3),
        iso3 != "",
  ...
  fetch_ppp_factors <- function(url, cache_path) {
    fetched <- tryCatch(
      {
        ppp_json <- perform_get_json(url)
  
        bind_rows(lapply(ppp_json[[2]], function(x) {
          tibble(
          ...
          mutate(iso3 = str_to_upper(str_squish(iso3))) |>
          group_by(iso3) |>
          slice_max(ppp_year, n = 1, with_ties = FALSE) |>
          ungroup()
      },
      error = function(e) {
        if (file.exists(cache_path)) {
          warning(
            paste0(
              "Nie udało się pobrać danych PPP z API World Bank. ",
              "Używam lokalnego cache: ", cache_path,
              "\nSzczegóły błędu: ", conditionMessage(e)
            ),
            call. = FALSE
          )
          return(read_ppp_cache(cache_path))
        }
  
        stop(
          paste0(
            "Nie udało się pobrać danych PPP z API World Bank i nie znaleziono cache: ",
            cache_path,
            "\nUruchom skrypt ponownie później albo dodaj plik cache z kolumnami: ",
            "iso3, ppp_year, ppp_lcu_per_int_dollar.",
            "\nSzczegóły błędu: ", conditionMessage(e)
          ),
          call. = FALSE
        )
  ...
  ppp_tbl <- fetch_ppp_factors(ppp_url, ppp_cache_path)
  ...
  ggsave(
    filename = file.path(www_dir, "ikea_static_price_map_ggplot_sf_eur.png"),
    plot = map_ggplot,
    width = 12,
    height = 7,
    dpi = 300
  ...
  ggsave(
    filename = file.path(www_dir, "ikea_static_price_map_ggplot_sf_ppp.png"),
    plot = map_ggplot_ppp,
    width = 12,
    height = 7,
    dpi = 300,
  ...
  saveWidget(
    fig_map,
    file = file.path(www_dir, "ikea_interactive_choropleth_price_map_eur.html"),
    selfcontained = TRUE
  )
  ...
  saveWidget(
    fig_ranking,
    file = file.path(www_dir, "ikea_price_ranking_plotly_eur.html"),
    selfcontained = TRUE
  )
  ...
  saveWidget(
    fig_boxplot,
    file = file.path(www_dir, "ikea_price_boxplot_plotly_eur.html"),
    selfcontained = TRUE
  )
  ...
  write_csv(
    country_prices,
    file.path(data_dir, "ikea_country_price_statistics_eur.csv")
  )
  
  write_csv(
    ikea_eur,
    file.path(data_dir, "ikea_product_catalog_with_eur_prices.csv")
  )
  
  write_csv(
    ppp_tbl,
    file.path(data_dir, "ppp_conversion_factors_private_consumption.csv")
  )
  
  write_csv(
    ppp_top_countries,
    file.path(data_dir, "ikea_ppp_top_15_countries_diagnostics.csv")
  )
  
  write_csv(
    country_prices_ppp,
    file.path(data_dir, "ikea_country_price_statistics_ppp.csv")
  )
  
  write_csv(
    ikea_ppp,
    file.path(data_dir, "ikea_product_catalog_with_ppp_prices.csv")
  )
  ...
  invisible(list(
    ...
    output_www_dir = www_dir
  ))
}

# Intencjonalnie nie uruchamiamy pipeline automatycznie.
# Shiny automatycznie wykonuje source() dla plików w folderze R/.
# Automatyczne uruchomienie tutaj powodowałoby pobieranie API i generowanie plików przy starcie aplikacji.
